...
let modalTimer = null;
let lastSuspectCount = null;  // 직전 폴링의 누적 의심 건수 (null = 최초 로드 전)
...
/* ---------- 의심 건수 증가 감지 → 모달 팝업 ----------
   누적 의심 건수(suspect)가 직전 폴링보다 늘어나면 가장 최신 의심 건으로 팝업.
   최초 로드 시점의 기존 건수는 기준값으로만 잡고 팝업하지 않음.
*/
function detectNewSuspects() {
  ...
  // 최초 로드: 기준값만 설정, 팝업 없음
  if (lastSuspectCount === null) {
    lastSuspectCount = count;
    return;
  }

  // 의심 건수가 늘어났으면 → 가장 최신 의심 건으로 팝업
  if (count > lastSuspectCount && suspects.length) {
    const latest = suspects[0];  // allLogs는 최신순이므로 앞쪽이 최신
    showModal({
      gate: latest.gate,
